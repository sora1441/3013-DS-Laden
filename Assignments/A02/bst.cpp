...
//3013 Adv Algorithms
//A02
//This program sorts data from a text file and creates a 
//complete and balanced BST
...
void balanceTree (vector <int> btree, BSTree &BST, int front, int back)
{
    //base case
    if(front<=back)
    {
    //getting middle value
        ...
        BST.insert(btree[mid]);
        balanceTree(btree, BST, front, mid - 1);
		balanceTree(btree, BST, mid + 1, back);
    }
}

int main()
{
    int Tsize; //size of the vector
    vector <int> btree;
	BSTree B;
	...
	int data; //read from textfile
	while (infile >> data)
	{
		btree.push_back(data);
	}
	//sort
	sort(btree.begin(), btree.end());
	Tsize = btree.size(); //gives value to size
	
	//Print number from file into the vector
	for (int i = 0; i < Tsize; i++) {
		//B.insert(btree[i]);
		cout << btree[i] << " ";
	}
	//balance the tree
	balanceTree(btree, B, 0, Tsize - 1);
	infile.close(); //Close the file	
	cin.get(); //waits till enter is hit
	B.printLevelOrder();
